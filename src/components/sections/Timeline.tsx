      ...
      <div className="container">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {timelineData.map((item, index) => (
            <div key={index} className="p-6 rounded-lg shadow-lg bg-card border-border border transition-all duration-300 hover:border-primary/20 hover:shadow-xl hover:-translate-y-1">
              <div className='flex items-center justify-between gap-4'>
                <div className='space-y-1'>
                    <h3 className="text-xl font-semibold text-foreground">{item.title}</h3>
                    <p className="text-sm font-medium text-primary">{item.date}</p>
                </div>
                 <div className="w-10 h-10 flex-shrink-0 bg-background border border-primary rounded-full flex items-center justify-center text-primary shadow-sm transition-all duration-300 group-hover:scale-105">
                  {item.icon}
                </div>
              </div>
